...
impl Dir for Vec<Vec<MapElement>> {
    fn map_move(&self, dir: MapDirection, curr: &MapElement) -> Option<&MapElement> { 
        ...
        let el = match dir {
            MapDirection::Up => if point.0 == 0 { None } else { Some(&self[point.0 - 1][point.1]) },
            MapDirection::Left => if point.1 == 0 { None } else { Some(&self[point.0][point.1 - 1]) },
            MapDirection::Right => if point.1 == self.len() - 1 { None } else { Some(&self[point.0][point.1 + 1]) },
            MapDirection::Down => if point.0 == self[point.0].len() - 1 { None } else { Some(&self[point.0 + 1][point.1]) },
        };
